...
const AddMaintenanceModal: React.FC<AddMaintenanceModalProps> = ({ isOpen, onClose, onSave, assetId }) => {
  // FIX: Get suppliers from app context to populate the dropdown.
  const { suppliers } = useApp();
  const initialState = {
    description: '',
    cost: 0,
    date: new Date().toISOString().split('T')[0],
    // FIX: Changed 'supplier' to 'supplierId' to match the type definition.
    supplierId: '',
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-lg" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* FIX: Changed from text input to a select dropdown for suppliers. */}
          <div>
            <label htmlFor="supplierId" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Fornecedor (Opcional)</label>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label htmlFor="date" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Data do Serviço</label>
              <input type="date" name="date" id="date" value={maintenanceData.date} onChange={(e) => handleChange(e as any)} required className="input-style" />
            </div>
          </div>
